...
def draw_and_save(y_true=None, y_hat=None):
    ...
    if y_true is None or y_hat is None:
        ...
        label_path = os.path.join(opt.clinical_data_saved_base_path, 'y_true.csv')
        pred_path = os.path.join(opt.clinical_data_saved_base_path, 'y_hat.csv')
        y_true_df = pd.read_csv(label_path)
        y_hat_df = pd.read_csv(pred_path)
        y_true = y_true_df.values[::, 1]
        y_hat = y_hat_df.values[::, 1]
    else:
        label_path = os.path.join(opt.clinical_data_saved_base_path)
        pred_path = os.path.join(opt.clinical_data_saved_base_path)
    ...
    fig_save_path = os.path.join(opt.clinical_data_saved_base_path)
...
if __name__ == "__main__":
    ...
    test_set = Clinical_Dataset(opt, opt.test_set_path, opt.test_set_path)
    test_iter = DataLoader(test_set, batch_size=opt.single_ct_batch_size, shuffle=False)
    net = get_mlp(opt, True)
    net.to(opt.device)
