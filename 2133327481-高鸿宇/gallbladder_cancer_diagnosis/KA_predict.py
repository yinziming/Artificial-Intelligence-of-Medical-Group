...
def predict(net, test_iter, opt):
    ...
    net.eval()
    with torch.no_grad():
        for ct, clinical, y in tqdm.tqdm(test_iter):
            ct, clinical = ct.to(opt.device), clinical.to(opt.device)
            y_pred= net(ct, clinical)
...
def draw_and_save(y_true=None, y_hat=None):
    ...
    if y_true is None or y_hat is None:
        ...
        label_path = os.path.join(opt.KA_data_saved_base_path, f'ct_encoder_type_{ct_encoder_type}', f'fusion_mode_{opt.KA_fusion_mode}', 'backbone', opt.backbone_name, 'y_true.csv')
        pred_path = os.path.join(opt.KA_data_saved_base_path, f'ct_encoder_type_{ct_encoder_type}', f'fusion_mode_{opt.KA_fusion_mode}', 'backbone', opt.backbone_name, 'y_hat.csv')
        y_true_df = pd.read_csv(label_path)
        y_hat_df = pd.read_csv(pred_path)
        y_true = y_true_df.values[::, 1]
        y_hat = y_hat_df.values[::, 1]
    else:
        label_path = os.path.join(opt.KA_data_saved_base_path, f'ct_encoder_type_{ct_encoder_type}', f'fusion_mode_{opt.KA_fusion_mode}', 'backbone', opt.backbone_name)
        pred_path = os.path.join(opt.KA_data_saved_base_path, f'ct_encoder_type_{ct_encoder_type}', f'fusion_mode_{opt.KA_fusion_mode}', 'backbone', opt.backbone_name)
    ...
    fig_save_path = os.path.join(opt.KA_data_saved_base_path, f'ct_encoder_type_{ct_encoder_type}', f'fusion_mode_{opt.KA_fusion_mode}', 'backbone', opt.backbone_name)
...
if __name__ == "__main__":
    ...
    test_set = KA_ct_Dataset(opt, opt.KA_ct_testset_path, opt.KA_dataset_testset_path, 'test_set')
    test_iter = DataLoader(test_set, batch_size=opt.KA_batch_size, shuffle=False)
    net = get_ka_model(opt, load_weights=True)
    net.to(opt.device)
