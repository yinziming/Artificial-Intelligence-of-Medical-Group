...
from get_data import Multi_ct_Dataset
from torch.utils.data import DataLoader
from models.get_net import get_multi_slice_model
...
def draw_and_save(y_true=None, y_hat=None):
    ...
    if y_true is None or y_hat is None:
        ...
        label_path = os.path.join(opt.multi_ct_data_saved_base_path, 'backbone', opt.backbone_name, f'{opt.n_slice}slice', f'fusion_mode_{opt.fusion_mode}', 'y_true.csv')
        pred_path = os.path.join(opt.multi_ct_data_saved_base_path, 'backbone', opt.backbone_name, f'{opt.n_slice}slice', f'fusion_mode_{opt.fusion_mode}', 'y_hat.csv')
        y_true_df = pd.read_csv(label_path)
        y_hat_df = pd.read_csv(pred_path)
        y_true = y_true_df.values[::, 1]
        y_hat = y_hat_df.values[::, 1]
    else:
        label_path = os.path.join(opt.multi_ct_data_saved_base_path, 'backbone', opt.backbone_name, f'{opt.n_slice}slice', f'fusion_mode_{opt.fusion_mode}')
        pred_path = os.path.join(opt.multi_ct_data_saved_base_path, 'backbone', opt.backbone_name, f'{opt.n_slice}slice', f'fusion_mode_{opt.fusion_mode}')
    ...
    plt.xlim([0.0,1.0])
    plt.ylim([0.0,1.0])
    plt.plot(FPR,TPR,color='darkorange',label='AUC={:.3f}'.format(AUC))
    plt.plot([0, 1], [0, 1], color='navy',lw=3, linestyle='--')
    plt.yticks(size=10)
    plt.xticks(size=10)
    plt.legend(loc="lower right",fontsize=10)
    fig_save_path = os.path.join(opt.multi_ct_data_saved_base_path, 'backbone', opt.backbone_name, f'{opt.n_slice}slice', f'fusion_mode_{opt.fusion_mode}')
...
if __name__ == "__main__":
    ...
    test_set = Multi_ct_Dataset(opt.multi_ct_testset_path, 'test_set', opt.n_slice)
    test_iter = DataLoader(test_set, batch_size=opt.multi_ct_batch_size, shuffle=False)
    net = get_multi_slice_model(opt, load_weights=True)
