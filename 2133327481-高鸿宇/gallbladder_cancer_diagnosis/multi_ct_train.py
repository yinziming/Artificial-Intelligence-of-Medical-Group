...
def train(net:nn.Module, train_iter:DataLoader, valid_iter:DataLoader, opt:object, loss:nn, optimizer:torch.optim):
    ...
    weight_dir = os.path.join(opt.multi_ct_weight_dir, f'{opt.n_slice}slice', f'fusion_mode_{opt.fusion_mode}', backbone_name)
    log_dir = os.path.join(opt.multi_ct_log_dir, f'{opt.n_slice}slice', f'fusion_mode_{opt.fusion_mode}', backbone_name)
    ...
    for epoch in range(opt.multi_ct_epoch):
        ...
        if min_val_loss > valid_loss:
            ...
            best_weight_dir = os.path.join(weight_dir, 'multi_ct_model' + '.pth.tar')
...
def adjust_learning_rate(optimizer, epoch, opt):
    ...
    lr = opt.multi_ct_lr
    lr *= 0.5 * (1. + math.cos(math.pi * epoch / opt.multi_ct_lr))
    for param_group in optimizer.param_groups:
        param_group['lr'] = lr

if __name__ == "__main__":
    train_set = Multi_ct_Dataset(opt.multi_ct_path, 'train_set', n_slice=opt.n_slice)
    valid_set = Multi_ct_Dataset(opt.multi_ct_path, 'valid_set', n_slice=opt.n_slice)

    train_iter = DataLoader(train_set, batch_size=opt.multi_ct_batch_size, shuffle=True)
    valid_iter = DataLoader(valid_set, batch_size=opt.multi_ct_batch_size, shuffle=False)

    net = get_multi_slice_model(opt)
    print(net)
    
    loss = LabelSmoothingCrossEntropy()

    optim = torch.optim.Adam(net.parameters(), lr=opt.multi_ct_lr)
